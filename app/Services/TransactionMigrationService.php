...
class TransactionMigrationService extends MigrationService{
    ...
    public static function migrateTransaction1():void{
      ...
      $pendingTransactions = Transaction::where('status', 0)->where('cron_id', 1)->select('member_reference', 'status', 'product_code', 'branch_code', 'account_number', 'transaction_date', 'dumped_date', 'transaction_reference', 'channel', 'product_code', 'transaction_type')->limit(1000);
          ...
          foreach($pendingTransactions->get() as $pendingTransaction){
            ...
            $membership_id_resolved = parent::string_encrypt(parent::resolveMemberReference($pendingTransaction->member_reference), self::$key,self::$iv);
              $arrayToPush = array(
                ...
                $checkAccNos = Enrollment::where('account_number',$pendingTransaction->account_number)
                                         ->where('member_reference', $pendingTransaction->member_reference)->select('member_reference','account_number')->first();
                //
                if($checkAccNos == null)
                {
            ...
            if ($repsonse['status'] == 1001){
                ...
                if(intval($values[2])>0){
                    EmailDispatcher::pendMails(parent::resolveMemberReference(trim($pendingTransaction->member_reference)), "YOU JUST EARNED LOYALTY POINTS ON THE FIRST GREEN REWARDS PROGRAMME", EmailDispatcher::buildTransactionTemplate(self::$placeholders, $values), 'no-reply@greenrewards.com');
                }
